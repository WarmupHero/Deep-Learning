...
def classification_diagnostics(trainer, X_val):
    """
    Compute extra diagnostics for classification runs.
    ...
    -------
    dict
        Extra summary statistics about the predicted probabilities
        on the validation set.
    """
    # Get predicted probabilities from the trained model
    y_val_pred = trainer.predict(X_val).reshape(-1)

    # Convert probabilities into hard class labels using threshold 0.5
    predicted_labels = (y_val_pred >= 0.5).astype(int)

    return {
        "mean_prediction": float(np.mean(y_val_pred)),
        "min_prediction": float(np.min(y_val_pred)),
        "max_prediction": float(np.max(y_val_pred)),
        "predicted_ones": int(np.sum(predicted_labels)),
        "predicted_zeros": int(len(predicted_labels) - np.sum(predicted_labels))
    }
...
def run_single_experiment(
    ...
    # For classification only, add extra probability/prediction diagnostics
    if problem_name == "classification":
        summary.update(classification_diagnostics(trainer, X_val))
